...
const SpotifyCards = memo(
	({
		...
		if (size === "LARGE") {
			return (
				<div
					className={`size-full bg-[#EDFDF3] rounded-3xl p-4 border border-gray-200 flex flex-col relative group	`}>
					<div className="remove-card absolute top-[-10px] left-[-10px] border border-gray-200 bg-white rounded-full w-8 h-8 flex items-center justify-center p-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
						<Trash className="w-10 h-10 text-black" />
					</div>
		...
		if (size === "TALL") {
			return (
				<div
					className={`size-full bg-[#EDFDF3] rounded-3xl p-4 flex flex-col justify-between border border-gray-200 relative group`}>
					<div className="remove-card absolute top-[-10px] left-[-10px] border border-gray-200 bg-white rounded-full w-8 h-8 flex items-center justify-center p-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
						<Trash className="w-10 h-10 text-black" />
					</div>
					<div className="flex flex-row flex-1 justify-between items-start mb-4">
		...
		if (size === "SMALL") {
			return (
				<div
					className={`size-full bg-[#EDFEF3] rounded-3xl p-4 flex flex-col justify-between border border-gray-200 relative group`}>
					<div className="remove-card absolute top-[-10px] left-[-10px] border border-gray-200 bg-white rounded-full w-8 h-8 flex items-center justify-center p-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
						<Trash className="w-10 h-10 text-black" />
					</div>
					<div className="flex flex-col items-start gap-1">
